...
resource "minio_iam_policy" "tempo_access_policy" {
  name = "tempo-access-policy"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
...
resource "minio_iam_user_policy_attachment" "policy_attachment" {
  depends_on = [
    ...
    minio_iam_policy.tempo_access_policy,
  ]
  for_each = {
    ...
    "${minio_iam_user.users["tempo-user"].name}" = minio_iam_policy.tempo_access_policy.name,
  }
  user_name   = each.key
